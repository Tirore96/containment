...
let andP b1 b2 =
  if b1
  then if b2 then ReflectT else ReflectF
  else ReflectF
...
(** val eq_op0 :
    Equality.coq_type -> Equality.sort rel **)

let eq_op0 s =
  (Equality.eqtype_hasDecEq_mixin
    (Equality.coq_class s)).Coq_hasDecEq.eq_op

(** val eqP0 :
    Equality.coq_type -> Equality.sort
    eq_axiom **)

let eqP0 s =
  (Equality.eqtype_hasDecEq_mixin
    (Equality.coq_class s)).Coq_hasDecEq.eqP
...
module Coq_isSub =
 struct
  type ('t, 'sub_sort) axioms_ = { val_subdef : 
                                   ('sub_sort
                                   -> 't);
                                   coq_Sub : 
                                   ('t -> __
...
module SubType =
 struct
  ...
  (** val eqtype_isSub_mixin :
      'a1 pred -> ('a1, 'a2) axioms_ ->
      ('a1, 'a2) Coq_isSub.axioms_ **)
...
(** val sub0 :
    'a1 pred -> 'a1 SubType.coq_type -> 'a1
    -> 'a1 SubType.sort **)

let sub0 p s x =
  Coq_isSub.coq_Sub p
    (SubType.eqtype_isSub_mixin p
      (SubType.coq_class p s)) x
...
(** val inj_eqAxiom :
    Equality.coq_type -> ('a1 ->
    Equality.sort) -> 'a1 eq_axiom **)

let inj_eqAxiom eT f x y =
  iffP (eq_op0 eT (f x) (f y))
    (eqP0 eT (f x) (f y))
...
let pair_eq t1 t2 u v =
  (&&) (eq_op0 t1 (fst u) (fst v))
    (eq_op0 t2 (snd u) (snd v))

(** val pair_eqP :
    Equality.coq_type -> Equality.coq_type
    -> (Equality.sort * Equality.sort)
    eq_axiom **)

let pair_eqP t1 t2 __top_assumption_ =
  let _evar_0_ =
    fun x1 x2 __top_assumption_0 ->
      ...
      iffP
        ...
        (andP
          ...
          (eq_op0 t2 (snd (x1, x2))
            (snd (y1, y2))))
    in
    let (a0, b0) = __top_assumption_0 in
    _evar_0_ a0 b0
  in
  let (a0, b0) = __top_assumption_ in
  _evar_0_ a0 b0
...
let hB_unnamed_factory_41 t1 t2 =
  { Coq_hasDecEq.eq_op = (pair_eq t1 t2);
    Coq_hasDecEq.eqP = (pair_eqP t1 t2) }
...
(** val hB_unnamed_factory_1 :
    int Coq_hasDecEq.axioms_ **)

let hB_unnamed_factory_1 =
  { Coq_hasDecEq.eq_op = eqn;
    Coq_hasDecEq.eqP = eqnP }
...
let eqseqP t _top_assumption_ =
  ...
  in
  let _evar_0_0 =
    fun x1 s1 iHs __top_assumption_ ->
    let _evar_0_0 = ReflectF in
    let _evar_0_1 = fun x2 s2 ->
      let __top_assumption_0 = eqP0 t x1 x2
      in
      let _evar_0_1 = fun _ ->
        iffP (eqseq t s1 s2) (iHs s2)
      in
...
let hB_unnamed_factory_0 t =
  { Coq_hasDecEq.eq_op = (eqseq t);
    Coq_hasDecEq.eqP = (eqseqP t) }
...
| y :: s' ->
  ...
  (fun x -> (||) (eq_op0 t x y) (p x))

type seq_eqclass = Equality.sort list

(** val pred_of_seq :
    Equality.coq_type -> seq_eqclass ->
    Equality.sort pred_sort **)
...
| x :: s' ->
  if in_mem x
       (mem (seq_predType t) (Obj.magic s'))
  then undup t s'
  else x :: (undup t s')
...
module Choice =
 struct
  type 't axioms_ = { choice_hasChoice_mixin : 
                      't
...
(** val find_subdef0 :
    Choice.coq_type -> Choice.sort pred ->
    int -> Choice.sort option **)

let find_subdef0 s x x0 =
  Coq_hasChoice.find_subdef
    (Choice.coq_class s).Choice.choice_hasChoice_mixin
    x x0

(** val pCanHasChoice :
    Choice.coq_type -> ('a1 -> Choice.sort)
    -> (Choice.sort -> 'a1 option) -> 'a1
    Coq_hasChoice.phant_axioms **)

let pCanHasChoice t _ f' =
  let liftP = fun sP ->
    simplPred (fun x ->
      Option.apply sP false (f' x))
  in
  let sf = fun sP n0 ->
    Option.bind f'
      (find_subdef0 t
        (PredOfSimpl.coerce (liftP sP)) n0)
  in
...
let nat_hasChoice =
  let f = fun p n0 ->
    if p n0 then Some n0 else None
  in
...
(** val pickle0 :
    Countable.coq_type -> Countable.sort ->
    int **)
...
(** val unpickle0 :
    Countable.coq_type -> int ->
    Countable.sort option **)
...
module Coq_isCountable =
 struct
  ...
  (** val phant_Build :
      ('a1 -> int) -> (int -> 'a1 option) ->
      'a1 axioms_ **)

  let phant_Build pickle1 unpickle1 =
    { pickle = pickle1; unpickle =
      unpickle1 }
...
(** val pCanIsCountable :
    Countable.coq_type -> ('a1 ->
    Countable.sort) -> (Countable.sort ->
    'a1 option) -> 'a1
    Coq_isCountable.axioms_ **)

let pCanIsCountable t f f' =
  Coq_isCountable.phant_Build
    (comp (pickle0 t) f)
    (pcomp f' (unpickle0 t))
...
(** val hB_unnamed_mixin_108 :
    Countable.coq_type -> Countable.sort
    pred -> Countable.sort SubType.coq_type
    -> Choice.sort SubType.sort
    Coq_hasChoice.phant_axioms **)
...
(** val hB_unnamed_mixin_109 :
    Countable.coq_type -> Countable.sort
    pred -> Countable.sort SubType.coq_type
    -> (Equality.sort SubType.sort,
    Equality.sort) inj_type
    Coq_hasDecEq.axioms_ **)
...
(** val hB_unnamed_mixin_111 :
    Countable.coq_type -> Countable.sort
    pred -> Countable.sort SubType.coq_type
    -> (Countable.sort SubType.sort,
    Countable.sort) pcan_type
    Choice_isCountable.axioms_ **)
...
let hB_unnamed_factory_123 =
  { Choice_isCountable.pickle = (fun x ->
    x); Choice_isCountable.unpickle =
    (fun x -> Some x) }
...
module Finite =
 struct
  type 't axioms_ = { choice_hasChoice_mixin : 
                      ...
                      eqtype_hasDecEq_mixin : 
                      't Coq_hasDecEq.axioms_;
                      fintype_isFinite_mixin : 
                      't Coq_isFinite.axioms_ }
  ...
  module Finite =
   struct
    module type Coq_enumLocked =
     sig
      val body :
        Finite.coq_type -> Finite.sort list
     end
...
let hB_unnamed_factory_58 n0 =
  { Coq_isSub.val_subdef = (nat_of_ord n0);
    ...
    Coq_isSub.coq_Sub_rect = (fun _ k_S u ->
    k_S u __) }
...
let hB_unnamed_factory_60 n0 =
  Obj.magic Countable.coq_class
    (eqtype_sub_type_of__canonical__choice_Countable
      datatypes_nat__canonical__choice_Countable
      (fun x ->
      leq (Stdlib.Int.succ (Obj.magic x)) n0)
      (Obj.magic
        fintype_ordinal__canonical__eqtype_SubType
        n0))
...
module PolyType =
 struct
  ...
  | Coq_nil -> 0
  | Coq_cons (_, xs0) ->
    Stdlib.Int.succ (size xs0)
 end

type fin = __

(** val nth_fin :
    'a1 PolyType.seq -> fin -> 'a1 **)
...
let rec happ n0 x x0 =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> Obj.magic x)
    (fun n1 ->
    happ n1 (Obj.magic x (Obj.magic x0).hd)
      (Obj.magic x0).tl)
    n0

(** val hcurry :
    int -> ('a1 hlist -> 'a2) -> ('a1, 'a2)
    hfun **)
...
let rec hcurry n0 x =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    Obj.magic (fun x0 ->
      hcurry n1 (fun xs ->
        Obj.magic x { hd = x0; tl = xs })))
    n0

type ('t, 's) hfun2 = __

(** val happ2 :
    int -> (fin -> int) -> ('a1, 'a2) hfun2
    -> 'a1 hlist2 -> 'a2 **)
...
let rec happ2 n0 m x x0 =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    happ2 n1 (fun i -> Obj.magic m (Some i))
      (happ (Obj.magic m None) x
        (Obj.magic x0).hd) (Obj.magic x0).tl)
    n0

type ('t, 'x) hdfun = __

(** val hdapp :
    int -> ('a1, 'a2) hdfun -> 'a1 hlist ->
    'a2 **)
...
let rec hdapp n0 x x0 =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    let xs = (Obj.magic x0).tl in
    hdapp n1 (Obj.magic x x1) xs)
    n0
...
let rec hlist_of_fun n0 f =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> Obj.magic ())
    (fun n1 ->
    Obj.magic { hd = (Obj.magic f None);
      tl =
      (hlist_of_fun n1 (fun i ->
        Obj.magic f (Some i))) })
    n0

(** val hmap :
    int -> (fin -> 'a1 -> 'a2) -> 'a1 hlist
    -> 'a2 hlist **)
...
let rec hmap n0 f x =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    (fun n1 ->
    Obj.magic { hd =
      (Obj.magic f None (Obj.magic x).hd);
      tl =
      (hmap n1 (fun j ->
        Obj.magic f (Some j))
        (Obj.magic x).tl) })
...
(** val hmap' :
    ('a1 -> 'a2 -> 'a3) -> 'a1 PolyType.seq
    -> ('a1, 'a2) hlist' -> ('a1, 'a3) hlist' **)

let hmap' f xs =
  hmap (PolyType.size xs) (fun i ->
    f (nth_fin xs i))

type 'x hlist1' = __

(** val hnth1' :
    int -> 'a1 hlist1' -> fin -> 'a1 **)
...
(** val type_of_arg_map :
    int -> (fin -> 'a1 -> 'a2) -> arg -> 'a1
    type_of_arg -> 'a2 type_of_arg **)
...
(** val add_arity_ind :
    int -> (fin -> arity PolyType.seq) ->
    fin -> arity -> fin -> 'a1 -> 'a1 -> 'a1 **)
...
type ('k, 'sort) arg_inst = { arg_inst_sort : 
                              arg;
                              arg_inst_class : 
                              ('k, 'sort)
                              arg_class }
...
type ('k, 'sort) arity_inst = { arity_inst_sort : 
                                arity;
                                arity_inst_class : 
                                ('k, 'sort)
                                arity_class }
...
type ('k, 'sort) sig_inst = { sig_inst_sort : 
                              signature;
                              sig_inst_class : 
                              ('k, 'sort)
                              sig_class }
...
(** val untag_decl :
    int -> int -> tagged_decl -> fin ->
    signature **)

let untag_decl _ _ t =
  t

type ('k, 'sort) decl_inst = { decl_inst_sort : 
                               tagged_decl;
                               decl_inst__1 : 
                               (fin -> ('k,
                               'sort)
...
let nonRec_arg_inst _ sX =
  { arg_inst_sort = NonRec; arg_inst_class =
    (Obj.magic sX) }

(** val rec_arg_inst :
    int -> fin -> ('a1, 'a2) arg_inst **)

let rec_arg_inst _ i =
  { arg_inst_sort = (Rec i);
    arg_inst_class = (Obj.magic ()) }

(** val nil_arity_inst :
    int -> ('a1, 'a2) arity_inst **)

let nil_arity_inst _ =
  { arity_inst_sort = PolyType.Coq_nil;
    arity_inst_class = (Obj.magic ()) }

(** val cons_arity_inst :
    int -> ('a1, 'a2) arg_inst -> ('a1, 'a2)
    arity_inst -> ('a1, 'a2) arity_inst **)

let cons_arity_inst _ ai asi =
  { arity_inst_sort = (PolyType.Coq_cons
    (ai.arg_inst_sort,
    asi.arity_inst_sort));
    arity_inst_class =
    (Obj.magic { hd = ai.arg_inst_class;
      tl = asi.arity_inst_class }) }

(** val nil_sig_inst :
    int -> ('a1, 'a2) sig_inst **)

let nil_sig_inst _ =
  { sig_inst_sort = PolyType.Coq_nil;
    sig_inst_class = (Obj.magic ()) }

(** val cons_sig_inst :
    int -> ('a1, 'a2) arity_inst -> ('a1,
    'a2) sig_inst -> ('a1, 'a2) sig_inst **)

let cons_sig_inst _ asi _UU03a3_i =
  { sig_inst_sort = (PolyType.Coq_cons
    (asi.arity_inst_sort,
    _UU03a3_i.sig_inst_sort));
    sig_inst_class =
    (Obj.magic { hd = asi.arity_inst_class;
      tl = _UU03a3_i.sig_inst_class }) }

(** val nil_decl_tag :
    int -> int -> (fin -> signature) ->
    tagged_decl **)
...
(** val nil_decl_inst :
    int -> (fin -> signature) -> ('a1, 'a2)
    decl_inst **)

let nil_decl_inst n0 f =
  { decl_inst_sort = (nil_decl_tag n0 0 f);
    decl_inst__1 = (fun _ -> assert false
    (* absurd case *)) }

(** val cons_decl_inst :
    int -> int -> ('a1, 'a2) sig_inst ->
    ('a1, 'a2) decl_inst -> signature
    fun_split -> ('a1, 'a2) decl_inst **)

let cons_decl_inst n0 k _UU03a3_i di d =
  { decl_inst_sort =
    (cons_decl_tag n0 (Stdlib.Int.succ k)
      (fs_fun k _UU03a3_i.sig_inst_sort
        (untag_decl n0 k di.decl_inst_sort)
        d)); decl_inst__1 = (fun i ->
    match Obj.magic i with
    | Some i0 ->
      cast
        (untag_decl n0 k di.decl_inst_sort
  ...
  | PolyType.Coq_cons (a0, as1) ->
    (match a0 with
     | NonRec ->
       cast __ __
         (pNonRec
           (PolyType.sval (Obj.magic cAs).hd)
           as1
           (arity_rec n0 pnil pNonRec pRec
             as1 (Obj.magic cAs).tl))
     | Rec i ->
       pRec i as1
...
module Ind =
 struct
  ...
  type 't constructors =
    fin -> coq_Cidx -> (arg, 't type_of_arg,
    't) hfun'
  ...
  (** val add_cons :
      int -> declaration -> 'a1 constructors
      -> fin -> arity -> (arg, 'a1
      type_of_arg, 'a1) hfun' -> 'a1
      constructors **)
  ...
  type 't recursor =
    __ -> (('t, __) rec_branch, ('t -> __)
    hlist1) hfun2

  (** val rec_branch'_of_hfun' :
      int -> fin -> arity -> (arg,
      ('a1 * 'a2) type_of_arg, 'a2) hfun' ->
      ('a1, 'a2) rec_branch' **)
  ...
  type ('t, 's) des_branch =
    (arg, 't type_of_arg, 's) hfun'

  type 't destructor =
    __ -> (('t, __) des_branch, ('t -> __)
    hlist1) hfun2
  ...
  type 't induction =
    (__, (('t, __) ind_branch, ('t -> __)
    hlist1) hfun2) hdfun

  module Def =
   struct
    type 'sorts class_of = { coq_Cons : 
                             'sorts
                             constructors;
                             coq_rec : 
                             'sorts recursor;
    ...
    (** val decl :
        coq_type -> declaration **)

    let decl t =
      t.decl

    (** val coq_class :
        coq_type -> __ class_of **)

    let coq_class t =
      t.coq_class
   end

  type 't mixin_of = { def : Def.coq_type;
                       idx : fin }

  (** val def :
      'a1 mixin_of -> Def.coq_type **)
...
module IndF =
 struct
  ...
  (** val args :
      int -> declaration -> fin -> 'a1 fobj
      -> (arg, 'a1 type_of_arg) hlist' **)

  let args _ _ _ f =
    f.args

  (** val fmap :
      int -> declaration -> (fin -> 'a1 ->
      'a2) -> fin -> 'a1 fobj -> 'a2 fobj **)

  let fmap n0 d f i x =
    { constr = x.constr; args =
      (hmap' (type_of_arg_map n0 f)
        (nth_fin (d i) x.constr) x.args) }

  (** val coq_Roll :
      Ind.Def.coq_type -> fin -> __ fobj ->
      __ **)

  let coq_Roll t i x =
    happ
      (PolyType.size
        (nth_fin (t.Ind.Def.decl i) x.constr))
      (t.Ind.Def.coq_class.Ind.Def.coq_Cons
        i x.constr) x.args

  (** val rec_branches_of_fun :
      Ind.Def.coq_type -> (fin -> (__ * 'a1)
      fobj -> 'a1) -> (__, 'a1)
      Ind.rec_branch hlist2 **)
  ...
  let rec_branches_of_fun t body0 =
    hlist_of_fun t.Ind.Def.n (fun i ->
      hlist_of_fun
        ...
        (fun j ->
        Ind.rec_branch'_of_hfun' t.Ind.Def.n
          i (nth_fin (t.Ind.Def.decl i) j)
          (hcurry
            (PolyType.size
              (nth_fin (t.Ind.Def.decl i) j))
            (fun l ->
            body0 i { constr = j; args = l }))))

  (** val coq_rec :
      Ind.Def.coq_type -> (fin -> (__ * 'a1)
      fobj -> 'a1) -> (__ -> 'a1) hlist1 **)

  let coq_rec t body0 =
    happ2 t.Ind.Def.n (fun i ->
      PolyType.size (t.Ind.Def.decl i))
      (Ind.Def.coq_rec t.Ind.Def.n
        t.Ind.Def.decl t.Ind.Def.coq_class)
      (rec_branches_of_fun t body0)

  type 'r lift_type = __

  (** val lift_type_of :
      Ind.Def.coq_type -> fin -> fin -> (__
      -> 'a1) -> 'a1 lift_type **)
    ...
    cast __ __
      (hnth1 t.Ind.Def.n
        (happ2 t.Ind.Def.n (fun i0 ->
          ...
          (Ind.Def.case t.Ind.Def.n
            t.Ind.Def.decl
            t.Ind.Def.coq_class)
          (des_branches_of_fun t i body0)) i
        x)

  (** val indP :
      Ind.Def.coq_type -> (fin -> (__, 'a1)
      sigT fobj -> 'a1) -> fin -> __ -> 'a1 **)

  let indP t iH =
    let q =
      hlist_of_fun t.Ind.Def.n (Obj.magic __)
    in
    let q_of_P = fun _ _ -> cast __ __ in
    let p_of_Q = fun _ _ -> cast __ __ in
    let tP_of_TQ = fun i x ->
      tagged0 (tag x)
        (p_of_Q i (tag x) (tagged x))
    in
    let iH0 = fun i a0 ->
      q_of_P i
        (coq_Roll t i
          (fmap t.Ind.Def.n t.Ind.Def.decl
            (fun _ -> tag) i
            (fmap t.Ind.Def.n t.Ind.Def.decl
              tP_of_TQ i a0)))
        (iH i
    ...
    p_of_Q i x
      (let n0 = t.Ind.Def.n in
       ...
       in
       let cs =
         __top_assumption_.Ind.Def.coq_Cons
       in
       let indP0 =
         __top_assumption_.Ind.Def.indP
       in
       let hyps = fun i0 j ->
         let hyps = fun args0 ->
           iH0 i0 { constr = j; args =
             args0 }
         in
         let _evar_0_ =
           fun _ _view_subject_ ->
           _view_subject_ ()
         in
         let _evar_0_0 =
           fun __top_assumption_0 ->
           let _evar_0_0 =
             fun _ iH1 c hyps0 x0 ->
             iH1 (c x0) (fun args0 ->
               hyps0 { hd = x0; tl = args0 })
           in
           let _evar_0_1 =
             fun _ _ iH1 constr0 hyps0 x0 h ->
...
module DerEqType =
 struct
  (** val eq_op_branch :
      Ind.Def.coq_type -> arg PolyType.seq
      -> (arg, (Equality.coq_type,
      Equality.sort) arg_class) hlist' ->
      (arg, (__ * (__ -> bool)) type_of_arg)
      hlist' -> (arg, __ type_of_arg) hlist'
      -> bool -> bool **)
  ...
  let eq_op_branch t as0 cAs =
    arity_rec t.Ind.Def.n (fun _ _ b0 -> b0)
      ...
      rec0 (Obj.magic x).tl (Obj.magic y).tl
        ((&&) b0
          (eq_op0 r (Obj.magic x).hd
            (Obj.magic y).hd)))
      (fun _ _ rec0 x y b0 ->
      rec0 (Obj.magic x).tl (Obj.magic y).tl
        ((&&) b0
          (snd (Obj.magic x).hd
            (Obj.magic y).hd))) as0 cAs

  (** val eq_op :
      Ind.Def.coq_type -> (fin ->
      (Equality.coq_type, Equality.sort)
      sig_class) -> fin -> __ -> __ -> bool **)
        ...
        IndF.case t i (fun args2 ->
          match leq_fin
                  (PolyType.size
                    (t.Ind.Def.decl i))
                  args2.IndF.constr
                  args1.IndF.constr with
          | Inl _ ->
            eq_op_branch t
              (nth_fin (t.Ind.Def.decl i)
                args1.IndF.constr)
              (hnth
                (PolyType.size
                  (t.Ind.Def.decl i)) 
                (sT i) args1.IndF.constr)
              args1.IndF.args
              (cast args2.IndF.constr
                args1.IndF.constr
  ...
  (** val eq_opP :
      Ind.Def.coq_type -> (fin ->
      (Equality.coq_type, Equality.sort)
      sig_class) -> fin -> __ eq_axiom **)

  let eq_opP t sT i _top_assumption_ =
    let _evar_0_ =
      fun i0 __top_assumption_ ->
...
let regex_hasDecEq a0 =
  { Coq_hasDecEq.eq_op =
    ...
    (Obj.magic DerEqType.eq_opP
      (Ind.coq_class { Ind.def =
        { Ind.Def.n = (Stdlib.Int.succ 0);
        Ind.Def.decl =
        (add_arity (Stdlib.Int.succ 0)
          (add_arity (Stdlib.Int.succ 0)
            (add_arity (Stdlib.Int.succ 0)
              (add_arity (Stdlib.Int.succ 0)
                (add_arity (Stdlib.Int.succ
                  0)
                  (add_arity
                    (Stdlib.Int.succ 0)
                    (empty_decl
        ...
        Obj.magic (fun f f0 f1 f2 f3 f4 ->
          ...
          | Event s -> f1 s
          | Plus (r0, r1) ->
            f2 r0 (f5 r0) r1 (f5 r1)
          | Seq (r0, r1) ->
            f3 r0 (f5 r0) r1 (f5 r1)
          | Star r0 -> f4 r0 (f5 r0)
        ...
        (Obj.magic
          ...
          | Star r0 -> f4 r0 (f5 r0)
          in f5)) } }; Ind.idx =
        (Obj.magic None) }).Ind.def
      (decl_inst_class (Stdlib.Int.succ 0)
        (Stdlib.Int.succ 0)
        (cons_decl_inst (Stdlib.Int.succ 0)
          0
          (cons_sig_inst (Stdlib.Int.succ 0)
            (nil_arity_inst (Stdlib.Int.succ
              0))
            (cons_sig_inst (Stdlib.Int.succ
              0)
              (nil_arity_inst
                (Stdlib.Int.succ 0))
              (cons_sig_inst
                ...
                (cons_sig_inst
                  ...
                  (cons_sig_inst
                    ...
                    (cons_sig_inst
                      ...
                      (nil_sig_inst
                        (Stdlib.Int.succ 0))))))))
          (nil_decl_inst (Stdlib.Int.succ 0)
            (fun i ->
            add_arity (Stdlib.Int.succ 0)
              (add_arity (Stdlib.Int.succ 0)
                (add_arity (Stdlib.Int.succ
...
| Event e' ->
  if eq_op0 a0 e' e then Eps else Empt
| Plus (c0, c1) ->
  Plus ((derive a0 e c0), (derive a0 e c1))
| Seq (c0, c1) ->
  if nu a0 c0
  then Plus ((Seq ((derive a0 e c0), c1)),
         (derive a0 e c1))
  else Seq ((derive a0 e c0), c1)
| Star c0 ->
  Seq ((derive a0 e c0), (Star c0))
| _ -> Empt
...
| Event a' ->
  if eq_op0 a0 a1 a' then Eps :: [] else []
| Plus (r0, r1) ->
  cat (pd a0 a1 r0) (pd a0 a1 r1)
| Seq (r0, r1) ->
  if nu a0 r0
  then cat
         (map (fun x -> Seq (x, r1))
           (pd a0 a1 r0)) (pd a0 a1 r1)
  else map (fun x -> Seq (x, r1))
         (pd a0 a1 r0)
| Star r0 ->
  map (fun x -> Seq (x, (Star r0)))
    (pd a0 a1 r0)
| _ -> []

(** val pd_l :
    Equality.coq_type -> Equality.sort ->
    regex list -> Equality.sort list **)
...
(** val o :
    Equality.coq_type -> regex -> regex **)

let o a0 c =
  if nu a0 c then Eps else Empt

(** val o_l :
    Equality.coq_type -> pder -> regex **)
...
| P_pair (r0, r1, p0, p1) ->
  addn (pTree_1size a0 r0 p0)
    (pTree_1size a0 r1 p1)
| P_fold (r0, p0) ->
  pTree_1size a0 (Plus (Eps, (Seq (r0, (Star
    r0))))) p0
| _ -> Stdlib.Int.succ 0

(** val pTree_0size_rect :
    Equality.coq_type -> regex -> (pTree ->
    (pTree -> __ -> 'a1) -> 'a1) -> pTree ->
    'a1 **)

let rec pTree_0size_rect a0 r hsize u =
  let _evar_0_ = fun u' ->
    pTree_0size_rect a0 r hsize u'
  in
...
(** val pTree_case :
    Equality.coq_type -> regex -> (__ ->
    'a1) -> (Equality.sort -> __ -> 'a1) ->
    (regex -> regex -> __ -> pTree -> 'a1)
    -> (regex -> regex -> __ -> pTree ->
    'a1) -> (regex -> regex -> __ -> pTree
    -> pTree -> 'a1) -> (regex -> __ ->
    pTree -> 'a1) -> pTree -> 'a1 **)
...
(** val pair_pd_l :
    Finite.coq_type -> Equality.sort ->
    (pder * pder) -> Equality.sort
...
(** val reachable_wrap :
    Finite.coq_type -> pder -> pder -> (pder
    -> pder -> bool) -> bool **)
...
| Cplus of regex * regex * regex * regex
   * dsl * dsl
| Cseq of regex * regex * regex * regex
   * dsl * dsl
| Dsl_var of Equality.sort * Equality.sort
   * Equality.sort
...
let dropinv _ _ a0 b0 x =
  Ctrans ((Plus (Eps, (Seq (b0, (Star
    a0))))), (Plus (Eps, (Seq (a0, (Star
    a0))))), (Star a0), (Cplus (Eps, Eps,
    (Seq (b0, (Star a0))), (Seq (a0, (Star
    a0))), (Cid Eps), (Cseq (b0, a0, (Star
    a0), (Star a0), (Ctrans (b0, (Plus (b0,
    Eps)), a0, (TagL (b0, Eps)), (Ctrans
    ((Plus (b0, Eps)), (Plus (Eps, b0)), a0,
    (Retag (b0, Eps)), x)))), (Cid (Star
    a0)))))), (Wrap a0))

(** val interp :
    Finite.coq_type -> (regex * regex) list
    -> regex -> regex -> dsl -> pTree ->
    (Equality.sort -> Equality.sort -> __ ->
    pTree -> __ -> post) -> post **)
...
let rec interp af l _ _ p t f =
  ...
  | Shuffle (a0, b0, c) ->
    pTree_case
      (Finite.Exports.fintype_Finite__to__eqtype_Equality
        af) (Plus ((Plus (a0, b0)), c))
      (fun _ _ -> assert false
      (* absurd case *)) (fun _ _ _ ->
      assert false (* absurd case *))
      (fun _ _ _ p0 f0 ->
      pTree_case
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) (Plus (a0, b0)) (fun _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ -> assert false
        (* absurd case *))
        (fun _ _ _ p1 _ -> P_inl (a0, (Plus
        (b0, c)), p1)) (fun _ _ _ p1 _ ->
        P_inr (a0, (Plus (b0, c)), (P_inl
        (b0, c, p1)))) (fun _ _ _ _ _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ _ -> assert false
        (* absurd case *)) p0 f0)
      (fun _ _ _ p0 _ -> P_inr (a0, (Plus
      (b0, c)), (P_inr (b0, c, p0))))
  ...
  | Shuffleinv (a0, b0, c) ->
    pTree_case
      (Finite.Exports.fintype_Finite__to__eqtype_Equality
        af) (Plus (a0, (Plus (b0, c))))
      (fun _ _ -> assert false
      (* absurd case *)) (fun _ _ _ ->
      assert false (* absurd case *))
      (fun _ _ _ p0 _ -> P_inl ((Plus (a0,
      b0)), c, (P_inl (a0, b0, p0))))
      (fun _ _ _ p0 f0 ->
      pTree_case
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) (Plus (b0, c)) (fun _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ -> assert false
        (* absurd case *))
        (fun _ _ _ p1 _ -> P_inl ((Plus (a0,
        b0)), c, (P_inr (a0, b0, p1))))
        (fun _ _ _ p1 _ -> P_inr ((Plus (a0,
        b0)), c, p1)) (fun _ _ _ _ _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ _ -> assert false
        (* absurd case *)) p0 f0)
  ...
  | Retag (a0, b0) ->
    pTree_case
      ...
      assert false (* absurd case *))
      (fun _ _ _ -> assert false
      (* absurd case *)) (fun _ _ _ p0 _ ->
      P_inr (b0, a0, p0)) (fun _ _ _ p0 _ ->
      P_inl (b0, a0, p0))
      (fun _ _ _ _ _ _ -> assert false
      (* absurd case *)) (fun _ _ _ _ ->
      assert false (* absurd case *)) t f
  ...
  | Untag a0 ->
    pTree_case
      (Finite.Exports.fintype_Finite__to__eqtype_Equality
        af) (Plus (a0, a0)) (fun _ _ ->
      assert false (* absurd case *))
      (fun _ _ _ -> assert false
      (* absurd case *)) (fun _ _ _ p0 _ ->
      p0) (fun _ _ _ p0 _ -> p0)
      (fun _ _ _ _ _ _ -> assert false
      (* absurd case *)) (fun _ _ _ _ ->
      assert false (* absurd case *)) t f
  ...
  | Assoc (a0, b0, c) ->
    pTree_case
      ...
      pTree_case
        ...
        (fun _ _ _ _ _ -> assert false
        (* absurd case *))
        (fun _ _ _ p2 p3 _ -> P_pair (a0,
        (Seq (b0, c)), p2, (P_pair (b0, c,
        p3, p1)))) (fun _ _ _ _ ->
        assert false (* absurd case *)) p0 f0)
      (fun _ _ _ _ -> assert false
      (* absurd case *)) t f
  | Associnv (a0, b0, c) ->
    pTree_case
      ...
      pTree_case
        ...
        (fun _ _ _ _ _ -> assert false
        (* absurd case *))
        (fun _ _ _ p2 p3 _ -> P_pair ((Seq
        (a0, b0)), c, (P_pair (a0, b0, p0,
        p2)), p3)) (fun _ _ _ _ ->
  ...
  | Swap a0 ->
    pTree_case
      ...
      pTree_case
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) Eps (fun _ _ -> P_pair (Eps,
        a0, P_tt, p0)) (fun _ _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ _ _ -> assert false
        (* absurd case *)) (fun _ _ _ _ _ ->
        assert false (* absurd case *))
  ...
  | Swapinv a0 ->
    pTree_case
      ...
      pTree_case
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) Eps (fun _ _ -> P_pair (a0,
        Eps, p1, P_tt)) (fun _ _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ _ _ -> assert false
        (* absurd case *)) (fun _ _ _ _ _ ->
        assert false (* absurd case *))
  ...
  | DistL (a0, b0, c) ->
    pTree_case
      ...
      pTree_case
        ...
        (fun _ _ _ -> assert false
        (* absurd case *))
        (fun _ _ _ p2 _ -> P_inl ((Seq (a0,
        b0)), (Seq (a0, c)), (P_pair (a0,
        b0, p0, p2)))) (fun _ _ _ p2 _ ->
        P_inr ((Seq (a0, b0)), (Seq (a0,
        c)), (P_pair (a0, c, p0, p2))))
        (fun _ _ _ _ _ _ -> assert false
        (* absurd case *)) (fun _ _ _ _ ->
        assert false (* absurd case *)) p1 f0)
  ...
  | DistLinv (a0, b0, c) ->
    pTree_case
      (Finite.Exports.fintype_Finite__to__eqtype_Equality
        af) (Plus ((Seq (a0, b0)), (Seq (a0,
      c)))) (fun _ _ -> assert false
      (* absurd case *)) (fun _ _ _ ->
      assert false (* absurd case *))
      (fun _ _ _ p0 f0 ->
      pTree_case
        ...
        (fun _ _ _ _ _ -> assert false
        (* absurd case *))
        (fun _ _ _ p1 p2 _ -> P_pair (a0,
        (Plus (b0, c)), p1, (P_inl (b0, c,
        p2)))) (fun _ _ _ _ -> assert false
        (* absurd case *)) p0 f0)
      (fun _ _ _ p0 f0 ->
      pTree_case
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
  ...
  | DistR (a0, b0, c) ->
    pTree_case
      ...
      pTree_case
        ...
        (* absurd case *))
        (fun _ _ _ p2 _ -> P_inl ((Seq (a0,
        c)), (Seq (b0, c)), (P_pair (a0, c,
        p2, p1)))) (fun _ _ _ p2 _ -> P_inr
        ((Seq (a0, c)), (Seq (b0, c)),
        (P_pair (b0, c, p2, p1))))
        (fun _ _ _ _ _ _ -> assert false
        (* absurd case *)) (fun _ _ _ _ ->
  ...
  | DistRinv (a0, b0, c) ->
    pTree_case
      ...
      pTree_case
        ...
        (fun _ _ _ _ _ -> assert false
        (* absurd case *))
        (fun _ _ _ p1 p2 _ -> P_pair ((Plus
        (a0, b0)), c, (P_inl (a0, b0, p1)),
        p2)) (fun _ _ _ _ -> assert false
        (* absurd case *)) p0 f0)
      (fun _ _ _ p0 f0 ->
      pTree_case
  ...
  | Wrap a0 ->
    pTree_case
      (Finite.Exports.fintype_Finite__to__eqtype_Equality
        af) (Plus (Eps, (Seq (a0, (Star
      a0))))) (fun _ _ -> assert false
      (* absurd case *)) (fun _ _ _ ->
      assert false (* absurd case *))
      (fun _ _ _ p0 f0 ->
      pTree_case
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) Eps (fun _ _ -> P_fold (a0,
        (P_inl (Eps, (Seq (a0, (Star a0))),
        P_tt)))) (fun _ _ _ -> assert false
        (* absurd case *)) (fun _ _ _ _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ _ _ -> assert false
        (* absurd case *))
      ...
      pTree_case
        ...
        (* absurd case *)) (fun _ _ _ _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ p1 p2 _ -> P_fold (a0,
        (P_inr (Eps, (Seq (a0, (Star a0))),
        (P_pair (a0, (Star a0), p1, p2))))))
        (fun _ _ _ _ -> assert false
        (* absurd case *)) p0 f0)
      (fun _ _ _ _ _ _ -> assert false
      (* absurd case *)) (fun _ _ _ _ ->
  ...
  | Drop (a0, b0, d) ->
    pTree_1size_rect
      (Finite.Exports.fintype_Finite__to__eqtype_Equality
        af) (Star a0) (fun u x f0 ->
      pTree_case
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) (Star a0) (fun _ _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ _ -> assert false
        (* absurd case *))
        (fun _ _ _ _ _ _ -> assert false
        ...
        pTree_case
          (Finite.Exports.fintype_Finite__to__eqtype_Equality
            af) (Plus (Eps, (Seq (a0, (Star
          a0))))) (fun _ _ _ -> assert false
          (* absurd case *)) (fun _ _ _ _ ->
          assert false (* absurd case *))
          (fun _ _ _ p1 x1 f2 ->
          pTree_case
            (Finite.Exports.fintype_Finite__to__eqtype_Equality
              af) Eps (fun _ _ _ -> P_inl
            (Eps, (Seq (b0, (Star a0))),
            P_tt)) (fun _ _ _ _ ->
            assert false (* absurd case *))
            (fun _ _ _ _ _ _ -> assert false
          ...
          pTree_case
            ...
            pTree_case
              ...
              (fun _ _ _ p4 _ _ -> P_inr
              (Eps, (Seq (b0, (Star a0))),
              (P_pair (b0, (Star a0), p4,
              p3)))) (fun _ _ _ _ _ _ _ ->
              assert false
              (* absurd case *))
              (fun _ _ _ _ _ -> assert false
              (* absurd case *))
              __top_assumption_ __ __)
            (fun _ _ _ _ _ -> assert false
            (* absurd case *)) p1 x1 f2)
          (fun _ _ _ _ _ _ _ -> assert false
          (* absurd case *))
          (fun _ _ _ _ _ -> assert false
          (* absurd case *)) p0 x0 f1) u x f0)
      t f
  | Cid _ -> t
  | Ctrans (a0, b0, c, d, d0) ->
    let t' = interp af l a0 b0 d t f in
    let hf = fun x y t0 -> f x y __ t0 __ in
    interp af l b0 c d0 t'
      (fun x y _ t0 _ -> hf x y t0)
  | Cplus (a0, a', b0, b', d, d0) ->
    ...
    pTree_case
      ...
      (fun _ _ _ -> assert false
      (* absurd case *)) (fun _ _ _ p0 f0 ->
      let hf0 = fun x y t0 -> f0 x y __ t0 __
      in
      let t0 =
        h0 p0 (fun x y _ t0 _ -> hf0 x y t0)
      in
      P_inl (a', b', t0))
      (fun _ _ _ p0 f0 ->
      let hf = fun x y t0 -> f0 x y __ t0 __
      in
      let t1 =
        h1 p0 (fun x y _ t0 _ -> hf x y t0)
      in
      P_inr (a', b', t1))
      (fun _ _ _ _ _ _ -> assert false
      (* absurd case *)) (fun _ _ _ _ ->
  ...
  | Cseq (a0, a', b0, b', d, d0) ->
    ...
    pTree_case
      ...
      let hf0 = fun x y t0 -> f0 x y __ t0 __
      in
      let hf1 = fun x y t0 -> f0 x y __ t0 __
      in
      let t0' =
        h0 p0 (fun x y _ t0 _ -> hf0 x y t0)
      in
      let t1' =
        h1 p1 (fun x y _ t0 _ -> hf1 x y t0)
      in
      P_pair (a', b', t0', t1'))
      (fun _ _ _ _ -> assert false
      (* absurd case *)) t f
    ...
    pTree_case
      ...
      pTree_case
        ...
        let __top_assumption_ =
          f1 a1 b0 __ p1 __
        in
        P_pair ((Event a0), (Obj.magic b0),
        (P_singl a0), __top_assumption_))
        (fun _ _ _ _ _ -> assert false
        (* absurd case *)) (fun _ _ _ _ _ ->
        assert false (* absurd case *))
        (fun _ _ _ _ _ _ -> assert false
  ...
  | Dsl_fix (r0, r1, p0) ->
    pTree_0size_rect
      (Finite.Exports.fintype_Finite__to__eqtype_Equality
        af) r0 (fun hin iH hf ->
      Obj.magic interp af ((r0, r1) :: l) r0
        r1 p0 hin (fun x y _ t0 _ ->
        let b0 =
          eq_op0
            (datatypes_prod__canonical__eqtype_Equality
              (regex_regex__canonical__eqtype_Equality
...
let o_plus_r af c0 c1 _ =
  ...
  then let b1 =
       ...
       if b1
       then Untag Eps
       else Dsl_fix ((Plus (Eps, Empt)),
              Eps, (Dsl_fix ((Plus (Eps,
              Empt)), Eps, (Dsl_fix ((Plus
              (Eps, Empt)), Eps, (Ctrans
              ((Plus (Eps, Empt)), (Plus
              (Empt, Eps)), Eps, (Retag
              (Eps, Empt)), (UntagL
...
(** val ctrans1 :
    Finite.coq_type -> regex ->
    (regex * regex) list -> regex -> regex
    -> dsl -> dsl -> dsl **)
...
(** val split_plus_l :
    Finite.coq_type -> (regex * regex) list
    -> Equality.coq_type -> Equality.sort
    list -> (Equality.sort -> regex) ->
    (Equality.sort -> regex) -> dsl **)
...
let split_plus_l af r _ _l_ p p' =
  ...
  let _evar_0_0 = fun a0 l iH ->
    ctrans1 af (Plus ((Plus ((p a0),
      (p' a0))),
      (Coq_bigop.body Empt l (fun j ->
        BigBody (j, (fun x x0 -> Plus (x,
        x0)), true, (Plus ((p j), (p' j))))))))
      r (Plus ((p a0), (Plus ((p' a0),
      (Coq_bigop.body Empt l (fun j ->
        BigBody (j, (fun x x0 -> Plus (x,
        x0)), true, (Plus ((p j), (p' j))))))))))
      (Plus ((Plus ((p a0),
...
let eps_c_l af r r0 =
  Ctrans ((Seq (r, Eps)), (Seq (Eps, r)), r,
    (ctrans1 af (Seq (r, Eps)) r0 (Seq (Eps,
      r)) (Seq (Eps, r)) (Swap r) (Cid (Seq
      (Eps, r)))), (Proj r))
...
(** val eq_big_plus_c :
    Finite.coq_type -> Finite.sort list ->
    (Finite.sort -> regex) -> (Finite.sort
    -> regex) -> (regex * regex) list -> dsl
    ex_coerce -> dsl **)
...
let eq_big_plus_c af _l_ f1 f2 r =
  ...
  let _evar_0_0 = fun a0 l iH hin ->
    cplus1 af (f1 a0) (f2 a0) r
      ...
      (Coq_bigop.body Empt l (fun j ->
        BigBody (j, (fun x x0 -> Plus (x,
        x0)), true, (f2 j)))) (hin a0 __)
      (iH (fun a1 _ -> hin a1 __))
  in
  let rec f = function
  | [] -> _evar_0_
  | y :: l0 -> _evar_0_0 y l0 (f l0)
...
(** val plus_empt_l :
    Finite.coq_type -> Equality.coq_type ->
    (regex * regex) list -> Equality.sort
    list -> dsl **)
...
let plus_empt_l af _ r _top_assumption_ =
  ...
  let _evar_0_0 = fun _ l iH ->
    ctrans2 af (Plus (Empt,
      (Coq_bigop.body Empt l (fun j ->
        BigBody (j, (fun x x0 -> Plus (x,
        x0)), true, Empt))))) r (Plus (Empt,
      Empt)) Empt (Untag Empt)
      (cplus1 af Empt Empt r
        (Coq_bigop.body Empt l (fun j ->
          BigBody (j, (fun x x0 -> Plus (x,
          x0)), true, Empt))) Empt (Cid
        Empt) iH)
  in
  let rec f = function
  | [] -> _evar_0_
...
let big_event_notin_l af r _top_assumption_ a0 =
      ...
      (Coq_bigop.body Empt l (fun j ->
        ...
        (if eq_op0
              (Finite.Exports.fintype_Finite__to__eqtype_Equality
                af) a2 j
         then Eps
         else Empt)))))))) r (Plus (Empt,
      Empt)) Empt (Untag Empt)
      (cplus1 af (Seq ((Event a1), Empt))
        Empt r
        (Coq_bigop.body Empt l (fun j ->
          BigBody (j, (fun x x0 -> Plus (x,
          x0)), true, (Seq ((Event j),
  ...
  let rec f l a1 =
    ...
    | [] -> _evar_0_ a1
    | y :: l0 ->
      _evar_0_0 y l0 (fun a2 _ -> f l0 a2) a1
...
let big_event_in_l af r _top_assumption_ a0 =
  let _evar_0_ = fun _ -> assert false
    (* absurd case *)
  in
  let _evar_0_0 = fun a1 l iH a2 ->
    let b0 =
    ...
    then let _evar_0_0 = fun _ ->
           ...
           ctrans2 af (Plus ((Seq ((Event
             ...
             ((Event a1), (Event a1)))
             (Event a1) (Untag (Event a1))
             (cplus1 af (Seq ((Event a1),
               Eps)) (Event a1) r
               (Coq_bigop.body Empt l
                 (fun j -> BigBody (j,
                 (fun x x0 -> Plus (x, x0)),
         ...
         let _evar_0_1 = fun _ ->
           ctrans1 af (Plus ((Seq ((Event
             ...
             (cplus1 af (Seq ((Event a1),
               ...
               (big_event_notin_l af r
                 (Obj.magic l) a1)) (Dsl_fix
             ((Plus ((Event a1), Empt)),
             (Event a1), (Dsl_fix ((Plus
             ((Event a1), Empt)), (Event
             a1), (Dsl_fix ((Plus ((Event
             a1), Empt)), (Event a1),
             (Ctrans ((Plus ((Event a1),
             Empt)), (Plus (Empt, (Event
             a1))), (Event a1), (Retag
             ((Event a1), Empt)), (UntagL
             (Event a1))))))))))
         in
         if in_mem a1
              (mem
         ...
         ctrans1 af (Plus ((Seq ((Event a1),
           ...
           ((Seq ((Event a1), Empt)), (Event
           a2))) (Event a2)
           (cplus1 af (Seq ((Event a1),
             Empt)) (Seq ((Event a1), Empt))
             r
             (Coq_bigop.body Empt l
               (fun j -> BigBody (j,
               (fun x x0 -> Plus (x, x0)),
             ...
             (Cid (Seq ((Event a1), Empt)))
             iH0) (Dsl_fix ((Plus ((Seq
           ((Event a1), Empt)), (Event
           a2))), (Event a2), (Dsl_fix
           ((Plus ((Seq ((Event a1), Empt)),
           (Event a2))), (Event a2), (Ctrans
           ((Plus ((Seq ((Event a1), Empt)),
           (Event a2))), (Plus (Empt, (Event
           a2))), (Event a2), (Cplus ((Seq
           ((Event a1), Empt)), Empt, (Event
           a2), (Event a2), (AbortR (Event
           a1)), (Cid (Event a2)))), (UntagL
           (Event a2)))))))))
  in
  let rec f = function
...
let big_event_in_r af r _top_assumption_ a0 =
  ...
  let _evar_0_0 = fun a1 l iH a2 ->
    ...
    if b0
    then let _evar_0_0 = fun _ ->
           ctrans2 af (Event a1) r (Seq
             ((Event a1), Eps)) (Plus ((Seq
             ((Event a1), Eps)),
             (Coq_bigop.body Empt l
               (fun j -> BigBody (j,
               (fun x x0 -> Plus (x, x0)),
             ...
             ((Event a1), (Seq ((Event a1),
             Eps)), (Dsl_fix ((Event a1),
             (Seq ((Event a1), Eps)),
             (Dsl_fix ((Event a1), (Seq
             ((Event a1), Eps)), (Ctrans
             ((Event a1), (Seq (Eps, (Event
             a1))), (Seq ((Event a1), Eps)),
             (Projinv (Event a1)), (Swapinv
             (Event a1))))))))))
         in
         let _evar_0_1 = fun _ ->
           ctrans2 af (Event a1) r (Seq
...
let derive_unfold_coerce af __top_assumption_ =
    ...
    (Coq_bigop.body Empt (index_enum af)
      ...
      (derive
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) a0 Empt)))))))
  in
  let _evar_0_1 = fun s -> Dsl_fix ((Event
    s), (Plus
    ((o
       (Finite.Exports.fintype_Finite__to__eqtype_Equality
         af) (Event s)),
    (Coq_bigop.body Empt (index_enum af)
      ...
      (derive
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) a0 (Event s))))))))),
    (ctrans2 af (Event s) (((Event s), (Plus
      (Empt,
      (Coq_bigop.body Empt (index_enum af)
        (fun a0 -> BigBody (a0, (fun x x0 ->
                       ...
                       (ctrans1 af (Plus
                         ...
                         (Coq_bigop.body
                           ...
                           (derive
                             (Finite.Exports.fintype_Finite__to__eqtype_Equality
                               af) a0 r1))))))))))))))))
    else ctrans2 af (Seq (r0, r1)) [] (Plus
           (Empt,
           (Coq_bigop.body Empt
             (index_enum af) (fun i ->
             BigBody (i, (fun x x0 -> Plus
      ...
      (cplus2 af Eps Eps [] (Seq
        ...
        (ctrans2 af (Seq
          ...
          (eq_big_plus_c af (index_enum af)
            (fun a0 -> Seq ((Seq ((Event
            a0),
            (derive
              (Finite.Exports.fintype_Finite__to__eqtype_Equality
                af) a0 r0))), (Star r0)))
            (fun i -> Seq ((Event i), (Seq
            ((derive
               (Finite.Exports.fintype_Finite__to__eqtype_Equality
                 af) i r0), (Star r0))))) []
            (fun a0 _ -> Assoc ((Event a0),
            (derive
              (Finite.Exports.fintype_Finite__to__eqtype_Equality
                af) a0 r0), (Star r0))))
          (factor_seq_r_r af []
  ...
  | Event s -> _evar_0_1 s
  | Plus (r0, r1) ->
    _evar_0_2 r0 (f r0) r1 (f r1)
  | Seq (r0, r1) ->
    _evar_0_3 r0 (f r0) r1 (f r1)
  ...
  let _evar_0_ =
    ctrans2 af (Plus (Eps,
      (Coq_bigop.body Empt (index_enum af)
        ...
        a0), Empt))))))) [] (Plus (Eps,
      Empt)) Eps
      (ctrans2 af (Plus (Eps, Empt)) []
        (Plus (Empt, Eps)) Eps (UntagL Eps)
        (Retag (Eps, Empt)))
      (cplus1 af Eps Eps []
        (Coq_bigop.body Empt (index_enum af)
          (fun a0 -> BigBody (a0,
          (fun x x0 -> Plus (x, x0)), true,
          (Seq ((Event a0), Empt))))) Empt
        (Cid Eps)
        (ctrans1 af
          (Coq_bigop.body Empt
            (index_enum af) (fun a0 ->
...
let empt_Eps af l =
  ctrans1 af Empt l (Plus (Empt, Eps)) Eps
    (TagL (Empt, Eps))
    (ctrans1 af (Plus (Empt, Eps)) l (Plus
      (Eps, Empt)) Eps (Retag (Empt, Eps))
      (Dsl_fix ((Plus (Eps, Empt)), Eps,
      (Dsl_fix ((Plus (Eps, Empt)), Eps,
      (Dsl_fix ((Plus (Eps, Empt)), Eps,
      (Ctrans ((Plus (Eps, Empt)), (Plus
      (Empt, Eps)), Eps, (Retag (Eps,
      Empt)), (UntagL Eps))))))))))

(** val o_lP :
    Finite.coq_type -> regex list -> regex
    list -> (regex * regex) list -> dsl **)
...
let c_eq_cat1 af __top_assumption_ =
  let _evar_0_ = fun l1 _ -> UntagLinv
    (Coq_bigop.body Empt l1 (fun i ->
      BigBody (i, (fun x x0 -> Plus (x,
      x0)), true, i)))
  in
  let _evar_0_0 = fun a0 l x l1 l0 ->
    ctrans2 af (Plus (a0,
      (Coq_bigop.body Empt (cat l l1)
        (fun j -> BigBody (j, (fun x0 x1 ->
        Plus (x0, x1)), true, j))))) l0
      (Plus (a0, (Plus
      ((Coq_bigop.body Empt l (fun j ->
         BigBody (j, (fun x0 x1 -> Plus (x0,
         x1)), true, j))),
      (Coq_bigop.body Empt l1 (fun i ->
        BigBody (i, (fun x0 x1 -> Plus (x0,
        x1)), true, i))))))) (Plus ((Plus
      (a0,
      (Coq_bigop.body Empt l (fun j ->
        BigBody (j, (fun x0 x1 -> Plus (x0,
      ...
      (cplus1 af a0 a0 l0
        (Coq_bigop.body Empt (cat l l1)
          (fun j -> BigBody (j,
          (fun x0 x1 -> Plus (x0, x1)),
          true, j))) (Plus
        ((Coq_bigop.body Empt l (fun j ->
           BigBody (j, (fun x0 x1 -> Plus
           (x0, x1)), true, j))),
        (Coq_bigop.body Empt l1 (fun i ->
          BigBody (i, (fun x0 x1 -> Plus
          (x0, x1)), true, i))))) (Cid a0)
        (x l1 l0))
  in
  let rec f = function
  ...
  let _evar_0_4 = fun r x l a0 ->
    ctrans2 af (Seq
      ...
      ((Coq_bigop.body Empt
         (pd
           (Finite.Exports.fintype_Finite__to__eqtype_Equality
             af) a0 r) (fun a1 -> BigBody
         (a1, (fun x0 x1 -> Plus (x0, x1)),
         true, a1))), (Star r)))
      (Coq_bigop.body Empt
        (pd
          (Finite.Exports.fintype_Finite__to__eqtype_Equality
...
let c_eq_derive_pd_l2 af __top_assumption_ =
  ...
  let _evar_0_1 = fun s l a0 ->
    ...
    in
    if b0
    then ctrans1 af (Plus (Eps, Empt)) l
           (Plus (Empt, Eps)) Eps (Retag
           (Eps, Empt)) (UntagL Eps)
    else Cid Empt
  in
  let _evar_0_2 = fun r x r0 x0 l a0 ->
...
(** val c_eq_undup1 :
    Finite.coq_type -> Equality.sort list ->
    (regex * regex) list -> dsl **)

let c_eq_undup1 af __top_assumption_ =
  let _evar_0_ = fun _ -> Cid
    (Coq_bigop.body Empt [] (fun i ->
      BigBody (i, (fun x x0 -> Plus (x,
      x0)), true, i)))
  in
  let _evar_0_0 = fun a0 l x l0 ->
    ...
    then ctrans2 af
           ...
           ((Obj.magic a0),
           (Coq_bigop.body Empt
             (Obj.magic l) (fun j -> BigBody
             (j, (fun x0 x1 -> Plus (x0,
             x1)), true, j)))))
           (cplus1 af (Obj.magic a0)
             (Obj.magic a0) l0
             (Coq_bigop.body Empt
               (Obj.magic undup
                 (regex_regex__canonical__eqtype_Equality
  ...
  | [] -> _evar_0_
  | y :: l0 ->
    Obj.magic _evar_0_0 y l0 (f l0)
  in f __top_assumption_

(** val c_eq_undup2 :
    Finite.coq_type -> regex list ->
    (regex * regex) list -> dsl **)
...
let decomp_p1_aux af l =
  ctrans1 af
    (Coq_bigop.body Empt l (fun i -> BigBody
      (i, (fun x x0 -> Plus (x, x0)), true,
      i))) [] (Plus
    ((o
       (Finite.Exports.fintype_Finite__to__eqtype_Equality
         af)
       (Coq_bigop.body Empt l (fun i ->
         BigBody (i, (fun x x0 -> Plus (x,
         x0)), true, i)))),
    (Coq_bigop.body Empt (index_enum af)
      (fun a0 -> BigBody (a0, (fun x x0 ->
    ...
    (eq_big_plus_c af (index_enum af)
      ...
      (derive
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) i
        (Coq_bigop.body Empt l (fun i0 ->
          BigBody (i0, (fun x x0 -> Plus (x,
          x0)), true, i0)))))) (fun i -> Seq
      ((Event i),
      (Coq_bigop.body Empt
        (Obj.magic pd_l
          (Finite.Exports.fintype_Finite__to__eqtype_Equality
            af) i l) (fun i0 -> BigBody (i0,
        (fun x x0 -> Plus (x, x0)), true,
        i0))))) [] (fun a0 _ -> Cseq ((Event
      a0), (Event a0),
      (derive
        (Finite.Exports.fintype_Finite__to__eqtype_Equality
          af) a0
...
(** val dsl_mon :
    Finite.coq_type -> (regex * regex) list
    -> Equality.sort list -> regex -> regex
    -> dsl -> dsl **)

let dsl_mon _ l l' e f p =
  let _evar_0_ = fun _ a0 b0 c _ -> Shuffle
    (a0, b0, c)
  in
  let _evar_0_0 = fun _ a0 b0 c _ ->
    Shuffleinv (a0, b0, c)
  in
  let _evar_0_1 = fun _ a0 b0 _ -> Retag
    (a0, b0)
  in
  let _evar_0_2 = fun _ a0 _ -> UntagL a0 in
  let _evar_0_3 = fun _ a0 _ -> UntagLinv a0
  in
  let _evar_0_4 = fun _ a0 _ -> Untag a0 in
  let _evar_0_5 = fun _ a0 b0 _ -> TagL (a0,
    b0)
  in
  let _evar_0_6 = fun _ a0 b0 c _ -> Assoc
    (a0, b0, c)
  in
  let _evar_0_7 = fun _ a0 b0 c _ ->
    Associnv (a0, b0, c)
  in
  let _evar_0_8 = fun _ a0 _ -> Swap a0 in
  let _evar_0_9 = fun _ a0 _ -> Swapinv a0 in
  let _evar_0_10 = fun _ a0 _ -> Proj a0 in
  let _evar_0_11 = fun _ a0 _ -> Projinv a0
  in
  let _evar_0_12 = fun _ a0 _ -> AbortR a0 in
  let _evar_0_13 = fun _ a0 _ -> AbortRinv a0
  in
  let _evar_0_14 = fun _ a0 _ -> AbortL a0 in
  let _evar_0_15 = fun _ a0 _ -> AbortLinv a0
  in
  let _evar_0_16 = fun _ a0 b0 c _ -> DistL
    (a0, b0, c)
  in
  let _evar_0_17 = fun _ a0 b0 c _ ->
    DistLinv (a0, b0, c)
  in
  let _evar_0_18 = fun _ a0 b0 c _ -> DistR
    (a0, b0, c)
  in
  let _evar_0_19 = fun _ a0 b0 c _ ->
    DistRinv (a0, b0, c)
  in
  let _evar_0_20 = fun _ a0 _ -> Wrap a0 in
  let _evar_0_21 = fun _ a0 _ -> Wrapinv a0
  in
  let _evar_0_22 = fun _ a0 b0 _ x l'0 ->
    Drop (a0, b0, (x l'0 __))
  in
  let _evar_0_23 = fun _ a0 _ -> Cid a0 in
  let _evar_0_24 =
    fun _ a0 b0 c _ x _ x0 l'0 -> Ctrans
    (a0, a0, c, (Cid a0), (Ctrans (a0, a0,
    c, (Cid a0), (Ctrans (a0, b0, c,
    (x l'0 __), (x0 l'0 __))))))
  in
  let _evar_0_25 =
    fun _ a0 a' b0 b' _ x _ x0 l'0 -> Cplus
    (a0, a', b0, b', (x l'0 __), (x0 l'0 __))
  in
  let _evar_0_26 =
    fun _ a0 a' b0 b' _ x _ x0 l'0 -> Cseq
    (a0, a', b0, b', (x l'0 __), (x0 l'0 __))
  in
  let _evar_0_27 = fun _ a0 a1 b0 _ ->
    Dsl_var (a0, a1, b0)
  in
  let _evar_0_28 = fun _ a0 b0 _ x l'0 ->
    Dsl_fix (a0, b0,
    (x ((a0, b0) :: l'0) __))
  in
  let rec f0 r _ _ d l'0 =
    match d with
    | Shuffle (a0, b0, c) ->
      _evar_0_ r a0 b0 c l'0
    | Shuffleinv (a0, b0, c) ->
      _evar_0_0 r a0 b0 c l'0
    ...
    | Assoc (a0, b0, c) ->
      _evar_0_6 r a0 b0 c l'0
    | Associnv (a0, b0, c) ->
      _evar_0_7 r a0 b0 c l'0
    ...
    | DistL (a0, b0, c) ->
      _evar_0_16 r a0 b0 c l'0
    | DistLinv (a0, b0, c) ->
      _evar_0_17 r a0 b0 c l'0
    | DistR (a0, b0, c) ->
      _evar_0_18 r a0 b0 c l'0
    | DistRinv (a0, b0, c) ->
      _evar_0_19 r a0 b0 c l'0
    ...
    | Ctrans (a0, b0, c, d0, d1) ->
      _evar_0_24 r a0 b0 c d0 (fun l'1 _ ->
        f0 r a0 b0 d0 l'1) d1 (fun l'1 _ ->
        f0 r b0 c d1 l'1) l'0
    | Cplus (a0, a', b0, b', d0, d1) ->
      _evar_0_25 r a0 a' b0 b' d0
        (fun l'1 _ -> f0 r a0 a' d0 l'1) d1
        (fun l'1 _ -> f0 r b0 b' d1 l'1) l'0
    | Cseq (a0, a', b0, b', d0, d1) ->
      _evar_0_26 r a0 a' b0 b' d0
        (fun l'1 _ -> f0 r a0 a' d0 l'1) d1
        (fun l'1 _ -> f0 r b0 b' d1 l'1) l'0
    | Dsl_var (a0, a1, b0) ->
      _evar_0_27 r a0 a1 b0 l'0
    | Dsl_fix (a0, b0, d0) ->
      Obj.magic _evar_0_28 r a0 b0 d0
        (fun l'1 _ ->
...
(** val check_o :
    Finite.coq_type -> pder -> pder -> bool **)
...
(** val eQ_complete_aux :
    Finite.coq_type -> nType ->
    (pder * pder) list -> (regex * regex)
    list -> (__, dsl) sumboolT **)
...
let rec eQ_complete_aux af p v l =
  ...
  then LeftT __
         ...
         ((Coq_bigop.body Empt (fst p)
            (fun i -> BigBody (i,
            (fun x x0 -> Plus (x, x0)),
            true, i))),
         (Coq_bigop.body Empt (snd p)
           (fun i -> BigBody (i,
           (fun x x0 -> Plus (x, x0)), true,
           i))), (Ctrans
         ((Coq_bigop.body Empt (fst p)
          ...
          let x = fun a1 ->
            ...
            let x =
              Obj.magic eQ_complete_aux af
                (Obj.magic pair_pd_l af a1
                  (a0, b1)) ((a0, b1) :: v)
                (((Coq_bigop.body Empt a0
                    (fun i -> BigBody (i,
                    (fun x x0 -> Plus (x,
                    x0)), true, i))),
                (Coq_bigop.body Empt b1
                  (fun i -> BigBody (i,
                  (fun x x0 -> Plus (x,
                  x0)), true, i)))) :: l)
            in
            (match x with
             | LeftT _ ->
               Dsl_var (a1, (Obj.magic r0),
                 (Obj.magic r1))
             | RightT b2 ->
               Cseq ((Event a1), (Event a1),
                 r0, r1, (Cid (Event a1)),
                 b2))
          in
          let lA = index_enum af in
          let _evar_0_ = fun _ -> Cid Empt in
          let _evar_0_0 = fun a1 l0 x0 x1 ->
            Cplus ((Seq ((Event a1),
            (Coq_bigop.body Empt
              (Obj.magic pd_l
                (Finite.Exports.fintype_Finite__to__eqtype_Equality
                  af) a1 a0) (fun i ->
              BigBody (i, (fun x2 x3 -> Plus
              (x2, x3)), true, i))))), (Seq
            ((Event a1),
            (Coq_bigop.body Empt
              (Obj.magic pd_l
              ...
              BigBody (i, (fun x2 x3 -> Plus
              (x2, x3)), true, i))))),
            (Coq_bigop.body Empt l0
              (fun j -> BigBody (j,
              (fun x2 x3 -> Plus (x2, x3)),
              true, (Seq ((Event j),
              (Coq_bigop.body Empt
                (Obj.magic pd_l
                  (Finite.Exports.fintype_Finite__to__eqtype_Equality
...
let c_eq_completeness af c0 c1 =
  let _view_subject_ =
    bisim_c_eq af (c0 :: []) (c1 :: [])
  in
  Ctrans (c0, c0, c1, (Cid c0), (Ctrans (c0,
  (Plus (c0, Empt)), c1, (TagL (c0, Empt)),
  (Ctrans ((Plus (c0, Empt)), (Plus (c1,
  Empt)), c1, _view_subject_, (Ctrans ((Plus
  (c1, Empt)), (Plus (Empt, c1)), c1, (Retag
  (c1, Empt)), (UntagL c1))))))))
...
let synth_coercion af r0 r1 =
  let b0 =
    reachable_wrap af (r0 :: []) (r1 :: [])
      (check_o af)
  in
  if b0
  then Some (c_eq_completeness af r0 r1)
  else None

type myFin = ordinal

(** val synth_coercion_wrap :
    regex -> regex -> dsl option **)
...
| [] -> []
| a0 :: l' ->
  if leq (Stdlib.Int.succ (Stdlib.Int.succ
       a0)) n0
  ...
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    | Plus (r0, r1) ->
      (match quick_parse_aux n' l r0 with
       | Some p ->
         let (l', t) = p in
         Some (l', (P_inl (r0, r1, t)))
       | None ->
         (match quick_parse_aux n' l r1 with
          | Some p ->
            let (l', t) = p in
            Some (l', (P_inr (r0, r1, t)))
          | None -> None))
    | Seq (r0, r1) ->
      (match quick_parse_aux n' l r0 with
       | Some p ->
         ...
         (match quick_parse_aux n' l' r1 with
          | Some p0 ->
            let (l'', t') = p0 in
            Some (l'', (P_pair (r0, r1, t,
            t')))
          | None -> None)
       | None -> None)
    | Star r0 ->
      let o0 =
        quick_parse_aux n' l (Plus (Eps,
          (Seq (r0, (Star r0)))))
      in
      (match o0 with
       | Some a0 ->
         ...
         pTree_case
           (fintype_ordinal__canonical__eqtype_Equality
             (Stdlib.Int.succ
             (Stdlib.Int.succ 0))) (Plus
           (Eps, (Seq (r0, (Star r0)))))
           (fun _ -> assert false
           (* absurd case *)) (fun _ _ ->
           assert false (* absurd case *))
           (fun _ _ _ p -> Some (a1, (P_fold
           (r0, (P_inl (Eps, (Seq (r0, (Star
           r0))), p)))))) (fun _ _ _ p ->
           pTree_case
             (fintype_ordinal__canonical__eqtype_Equality
               (Stdlib.Int.succ
               (Stdlib.Int.succ 0))) (Seq
             (r0, (Star r0))) (fun _ ->
             assert false (* absurd case *))
             (fun _ _ -> assert false
             (* absurd case *))
             ...
             (fun _ _ _ _ -> assert false
             (* absurd case *))
             (fun _ _ _ p0 p1 -> Some (a1,
             (P_fold (r0, (P_inr (Eps, (Seq
             (r0, (Star r0))), (P_pair (r0,
             (Star r0), p0, p1))))))))
             (fun _ _ _ -> assert false
             (* absurd case *)) p)
           (fun _ _ _ _ _ -> assert false
           (* absurd case *)) (fun _ _ _ ->
           assert false (* absurd case *)) b0
       | None -> None))
    n0

(** val quick_parse :
    int -> myFin list -> regex -> pTree
    option **)
...
(** val parse_inc :
    int -> regex -> int list -> (int list,
    pTree) sum **)

let parse_inc n0 r l =
  match quick_parse n0
          (to_string
            (ord0 (Stdlib.Int.succ 0))
            (Obj.magic index_enum
              (fintype_ordinal__canonical__fintype_Finite
                (Stdlib.Int.succ
                (Stdlib.Int.succ 0)))) l) r with
  | Some t -> Inr t
  | None ->
    Inl
      (inc_seq
...
| Ctrans (a0, b0, c, d0, d1) ->
  addn (dsl_size l a0 b0 d0)
    (dsl_size l b0 c d1)
| Cplus (a0, a', b0, b', d0, d1) ->
  addn (dsl_size l a0 a' d0)
    (dsl_size l b0 b' d1)
| Cseq (a0, a', b0, b', d0, d1) ->
  addn (dsl_size l a0 a' d0)
    (dsl_size l b0 b' d1)
| Dsl_fix (a0, b0, d0) ->
  dsl_size ((a0, b0) :: l) a0 b0 d0
| _ -> Stdlib.Int.succ 0

(** val time_synth_coercion :
    regex -> regex -> dsl option **)
...
(** val synth_interp :
    int -> int list -> regex -> regex ->
    (int * (int * pTree) list) option **)
...
let big_nat =
  of_num_uint (UIntDecimal (D1 (D0 (D0 (D0
    (D0 (D0 (D0 (D0 Nil)))))))))

(** val tree_sizes : int list **)

let tree_sizes =
  (of_num_uint (UIntDecimal (D5 (D0 (D0 (D0
    (D0 Nil))))))) :: ((Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ
...
let rp5 =
  (('a'::('^'::('*'::(' '::('x'::(' '::('b'::('^'::('*'::(' '::('<'::('='::(' '::('('::('('::('1'::(' '::('+'::(' '::('a'::(')'::(' '::('x'::(' '::('('::('1'::(' '::('+'::(' '::('b'::(')'::(')'::('^'::('*'::[])))))))))))))))))))))))))))))))))),
    ((Seq ((Star a), (Star b))), (Seq ((Star
    (Plus (Eps, a))), (Plus (Eps, b))))))
...
let rp5' =
  (('('::('('::('1'::(' '::('+'::(' '::('a'::(')'::(' '::('x'::(' '::('('::('1'::(' '::('+'::(' '::('b'::(')'::(')'::('^'::('*'::(' '::('<'::('='::(' '::('a'::('^'::('*'::(' '::('x'::(' '::('b'::('^'::('*'::(' '::(' '::[])))))))))))))))))))))))))))))))))))),
    ((Seq ((Star (Plus (Eps, a))), (Plus
    (Eps, b)))), (Seq ((Star a), (Star b)))))

(** val rp6 : char list * (regex * regex) **)

let rp6 =
  (('a'::('^'::('*'::(' '::('x'::(' '::('('::('1'::(' '::('+'::(' '::('a'::(')'::(' '::('<'::('='::(' '::('a'::('^'::('*'::[])))))))))))))))))))),
    ((Seq ((Star a), (Plus (Eps, a)))),
    (Star a)))
...
let rp8' =
  ((' '::('b'::(' '::('x'::(' '::('a'::('^'::('*'::(' '::('<'::('='::(' '::('b'::(' '::('+'::(' '::('b'::(' '::('x'::(' '::('a'::('^'::('*'::[]))))))))))))))))))))))),
    ((Seq (b, (Star a))), (Plus (b, (Seq (b,
    (Star a)))))))

(** val rp9 : char list * (regex * regex) **)

let rp9 =
  (('('::('a'::(' '::('+'::(' '::('b'::(')'::('^'::('*'::(' '::('<'::('='::(' '::('('::('a'::('^'::('*'::(' '::('+'::(' '::('b'::('^'::('*'::(')'::('^'::('*'::[])))))))))))))))))))))))))),
    ((Star (Plus (a, b))), (Star (Plus
    ((Star a), (Star b))))))
...
(** val test1 :
    unit -> (int * (int * pTree) list) option **)
...
let test =
  (((((((((((((((((((test1, test1'), test2),
    ...
    test6'), test7), test7'), test8),
    test8'), test9), test9'), test10),
    test10')
