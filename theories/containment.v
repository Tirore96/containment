...
Require Import  Containment.syntax.
(*
...
Fixpoint Σ (A:Type) (l : list A) (f : A -> regex) : regex :=
match l with
| [] => Failure
| c ::l => f c _+_ (Σ l f)
end.
...
Add Parametric Relation : regex (co_eq)
...
Lemma Σederive_eq : forall es ps R e, (forall p : regex * regex, In p ps ->  (fst p) =(R)= (snd p)) -> e \ (Σe es (map fst ps)) =(R)= e \ (Σe es (map snd ps)).
Proof.
...
Qed.
*)
Definition fun_eq R (f0 f1 : EventType -> regex) := (forall a, f0 a =(R)= f1 a).
...
Add Parametric Relation R : (EventType -> regex) (@fun_eq R)
...
Definition seq_fun (f0 f1 : EventType -> regex) := fun a => f0 a _;_ f1 a.
...
Definition plus_fun (f0 f1 : EventType -> regex) := fun a => f0 a _+_ f1 a.
...
Definition par_fun (f0 f1 : EventType -> regex) := fun a => f0 a _*_ f1 a.
...
Lemma Σ_split_plus : forall R (A: Type) l (P P' : A -> regex),
...
Lemma Σ_factor_seq_r : forall R l (P: EventType -> regex) c,
...
Lemma Σ_factor_seq_l : forall R l (P: EventType -> regex) c,
...
Lemma Σ_factor_par_l : forall R l1 c (P' : EventType -> regex),
...
Lemma Σ_factor_par_r : forall R l1 c (P' : EventType -> regex),
...
Lemma Σ_par_ΣΣ : forall R l0 l1 (P0 P1 : EventType -> regex),
...
Lemma ΣΣ_prod_swap : forall R l0 l1 (P : EventType -> EventType -> regex), 
...
Lemma rewrite_c_in_fun : forall R (c c' : regex) , c = (R) = c' -> (fun _ : EventType => c) =(λ R)= (fun _ : EventType => c').
Proof.
...
(*
Definition Σ_fun (f : EventType -> EventType -> regex) := fun a  => Σ alphabet (f a).
...
Definition app a (f : EventType -> regex) := f a.
