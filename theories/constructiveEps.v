...
Inductive pTree {A : eqType} : @regex A -> Type := 
| p_tt : pTree Eps 
...
Fixpoint pflatten {A : eqType} {r : regex} (T : pTree r) : seq A := 
match T with 
| p_tt => nil 
| p_singl a => (a :: nil )
...
Fixpoint pTree_0size {A : eqType} (r : @regex A)  (p : pTree r) := 
match p with 
| p_tt => 0
| p_singl _ => 1
...
Fixpoint pTree_1size {A : eqType} (r : @regex A)  (p : pTree r) := 
match p with 
| p_tt => 1 
| p_singl _ => 1
...
Definition pRel {A : eqType} (r : @regex A) (p' p : pTree r) := pTree_0size p' < pTree_0size p.
Definition pRel0 {A : eqType} (r' r : @regex A) (p' : pTree r') (p : pTree r) := pTree_0size p' < pTree_0size p.
Definition pRel1 {A : eqType} (r' r : @regex A) (p' : pTree r') (p : pTree r) := pTree_1size p' < pTree_1size p.

Lemma pTree_0size_rect
     : forall {A : eqType} (r: @regex A) (P : pTree r -> Type),
...
Lemma pTree_0size0 : forall {A : eqType} (r : @regex A) (p : pTree r), pTree_0size p = 0 -> pflatten p = nil.
Proof.
move=> A r.
...
Lemma pTree_1size1 : forall {A : eqType} (r : @regex A) (p : pTree r), 0 < pTree_1size p. 
Proof.
move=> A r.
elim;ssa.  lia.
Qed.

Lemma pTree_false : forall {A : eqType} (r: @regex A) (p : pTree r), pTree_1size p <= 0 -> False. 
Proof.
intros. move: (pTree_1size1 p). lia.
Qed.

(* ala https://jamesrwilcox.com/dep-destruct.html*)
Lemma pTree_case : forall {A : eqType} (r : @regex A) (P : pTree r -> Type), (forall (pf :  Eps = r), P (@eq_rect _ _ pTree p_tt _ pf)) -> 
