...
Inductive c_eq ( l : seq (regex * regex)) : regex -> regex -> Prop :=
| c_plus_assoc c0 c1 c2 : l |- (c0 _+_ c1) _+_ c2 =R= c0 _+_ (c1 _+_ c2)
| c_plus_comm c0 c1: l |- c0 _+_ c1 =R= c1 _+_ c0
| c_plus_neut c : l |-  c _+_ Empt =R= c
| c_plus_idemp c  : l |- c _+_ c =R= c 
| c_seq_assoc c0 c1 c2  : l |- (c0 _;_ c1) _;_ c2 =R= c0 _;_ (c1 _;_ c2)
| c_seq_neut_l c : l |- (Eps _;_ c) =R= c 
| c_seq_neut_r c : l |-  c _;_ Eps =R= c 
| c_seq_failure_l c : l |- Empt _;_ c =R= Empt  
| c_seq_failure_r c :  l |-c _;_ Empt =R= Empt 
| c_distr_l c0 c1 c2 : l |- c0 _;_ (c1 _+_ c2) =R= (c0 _;_ c1) _+_ (c0 _;_ c2)
| c_distr_r c0 c1 c2 : l |- (c0 _+_ c1) _;_ c2 =R= (c0 _;_ c2) _+_ (c1 _;_ c2)

| c_unfold c : l |-  Eps _+_ (c _;_ Star c) =R= Star c (*New star rule 1*)
| c_star_plus c : l |-  Star (Eps _+_ c) =R= Star c (*New star rule 2*)
| c_refl c : l |- c =R= c
| c_sym c0 c1 (H: l |- c0 =R=c1) : l |- c1 =R=c0
| c_trans c0 c1 c2 (H1 : l |- c0 =R=c1) (H2 : l |- c1 =R=c2) : l |- c0 =R=c2
| c_plus_ctx c0 c0' c1 c1' (H1 : l |- c0 =R=c0') 
                             (H2 : l |- c1 =R=c1') : l |- c0 _+_ c1 =R=c0' _+_ c1'
| c_seq_ctx c0 c0' c1 c1'  (H1 : l |- c0 =R=c0') 
                             (H2 : l |- c1 =R=c1') : l |-  c0 _;_ c1 =R=c0' _;_ c1'
| c_star_ctx c0 c1         (H :  l |- c0 =R=c1) : l |- Star c0 =R= Star c1  (*new context rule*) 
| c_fix a c0 c1 : (c0, c1) \in l -> l |-  Event a _;_ c0 =R= Event a _;_ c1
| c_fix2 a c0 c1 :  (c0,c1)::l |- c0 =R= c1 ->  l |-  Event a _;_ c0 =R= Event a _;_ c1
...
(*Hint Constructors c_eq.*)
(*Check c_plus_assoc.
Definition c_plus_assoc_nil := c_plus_assoc nil.
Hint Resolve c_plus_assoc_nil.*)
...
| c_star_plus c : l |-  Star (Eps _+_ c) =R= Star c (*New star rule 2*)
| c_refl c : l |- c =R= c
| c_sym c0 c1 : l |- c0 =R=c1 -> l |- c1 =R=c0
| c_trans c0 c1 c2 (H1 : l |- c0 =R=c1) (H2 : l |- c1 =R=c2) : l |- c0 =R=c2
| c_plus_ctx c0 c0' c1 c1' (H1 : (c0 _+_ c1 , c0' _+_ c1')::l |- c0 =R=c0') 
                             (H2 : (c0 _+_ c1 , c0' _+_ c1'):: l |- c1 =R=c1') : l |- c0 _+_ c1 =R=c0' _+_ c1'
| c_seq_ctx c0 c0' c1 c1'  (H1 : (c0 _;_ c1 , c0' _;_ c1')::l |- c0 =R=c0') 
                             (H2 : (c0 _;_ c1 , c0' _;_ c1')::l |- c1 =R=c1') : l |-  c0 _;_ c1 =R=c0' _;_ c1'
| c_star_ctx c0 c1         (H :  (Star c0 , Star c1)::l |- c0 =R=c1) : l |- Star c0 =R= Star c1  (*new context rule*) 
| c_fix a c0 c1 : (c0, c1) \in l -> l |-  Event a _;_ c0 =R= Event a _;_ c1
 where "l |- c1 =R= c2" := (c_eq l c1 c2).
End Equivalence.
Hint Constructors c_eq.
...
Definition EQ c0 c1 := c_eq nil c0 c1.  
...
Check c_eq. Check c_sym. Check c_sym.
Add Parametric Relation R : regex (@c_eq R)
  reflexivity proved by (c_refl R)
  symmetry proved by (@c_sym _)
  transitivity proved by (@c_trans R)
  as regex_setoid.

Lemma EQ_refl : forall e, EQ e e.
Proof. move => e. con. Qed.

Lemma EQ_sym : forall e e', EQ e e' -> EQ e' e.
Proof. move => e e'. con=>//. Qed.

Lemma EQ_trans : forall e e' e'', EQ e e' -> EQ e' e'' -> EQ e e''.
Proof. move => e e' e''. eauto. Admitted. 
(*con=>//. eauto.  Qed.*)



Lemma c_ineq_mon : forall c0 c1 l0 l1, (forall x, x \in l0 -> x \in l1) ->  c0 =(l0)=c1 -> c0 =(l1)=c1.
Proof. Admitted.
(*simpl. intros.
elim: H0 l1 H;eauto.
intros.  apply:c_plus_ctx. apply:H. 
intros. rewrite !inE in H. de (orP H).
apply:H0. intros. rewrite !inE. de (orP H0).
intros. apply:c_seq_ctx. apply:H.
intros. rewrite !inE in H. de (orP H).
apply:H0. intros. rewrite !inE. de (orP H0).
intros. apply:c_star_ctx. apply:H0.
intros. rewrite !inE in H0. de (orP H0).
Qed.*)
Hint Resolve c_ineq_mon : paco.

Add Parametric Morphism R : Plus with
signature (c_eq R) ==> (c_eq R) ==> (c_eq R) as c_eq_plus_morphism.
Proof.
intros. apply:c_plus_ctx.
eauto with paco. eauto with paco.
Qed.

Ltac ep := eauto with paco.

Add Parametric Morphism R : Seq with
signature (c_eq R) ==> (c_eq R) ==> (c_eq R) as co_eq_seq_morphism.
Proof.
intros. apply:c_seq_ctx;ep.
Qed.

Add Parametric Morphism R : Star with
signature (c_eq R) ==> (c_eq R) as c_eq_star_morphism.
Proof.
intros. apply:c_star_ctx;ep.
Qed.

Lemma c_plus_neut_l : forall R c, Empt _+_ c =(R)= c.
Proof. intros. 
rewrite c_plus_comm. auto.
Qed.
...
(*Hint Resolve c_ineq_mon.*)
Definition sub {B : eqType} (l0 l1 : seq B) := forall x, x \in l0 -> x \in l1.

Lemma sub_nil : forall (B : eqType) (l : seq B), sub nil l.
Proof. eauto.
intros. intro. ssa. 
Qed.

(*Lemma co_eq_derive : forall (c0 c1 : regex) e, c0 =(nil)= c1 -> e \ c0 =(nil)= e \ c1.
Proof.
move=> c0 c1 /= e.   move Hl: nil => l Heq.  
move: Heq e Hl. (**suff: c0 = (l)= c1 -> forall e : A, sub [::] l -> e \ c0 = (l)= e \ c1. *)
elim;intros;subst;try solve [ simpl; rewrite ?eqs;eauto] .
- rewrite /=. case Hcase: (nu c2)=>//=; case Hcase1: (nu c3)=>//=; rewrite !eqs;eq_m_left. 
- rewrite /=; case Hcase:(nu _)=>//=; rewrite !eqs //.
- rewrite /=; case Hcase: (nu _)=>//;rewrite !eqs //.  
- rewrite /=; case Hcase: (nu _)=>//;rewrite !eqs //. eq_m_left. eauto.
(*  rewrite c_plus_comm. eq_m_left.*)
- rewrite /=. case Hcase:(nu c2)=>//=;case Hcase':(nu c3)=>/=//;rewrite !eqs;eq_m_left. 
  rewrite {2}c_plus_comm -c_plus_assoc eqs c_plus_comm //. 
- rewrite /= eqs; case Hcase:(nu _)=>/=;rewrite  ?eqs //. 
- rewrite /=. 
  have: nu c2 = nu c0' by apply/c_eq_nu; eauto. move=>->.
  case Hcase:(nu _)=>//=;subst. eauto.  eauto. 
- done. 
-  rewrite /=. case Hcase: (_==_)=>//. rewrite
(* norm_eqs. rewrite !eqs. apply:c_ineq_mon=>/=. 2: eauto. done.*)
- simpl.
apply:H0.
apply:c_ineq_mon. 2: {  apply/H0.
rewrite !eqs //. 
- *)

Lemma sub_cat :  forall (B : eqType) (x:B) (x0 x1: seq B) x1, x \in x0 -> x \in x0 ++ x1.
Proof.
intros. rewrite mem_cat H //. 
Qed.

Lemma sub_cat2 :  forall (B : eqType) (x:B) (x0 x1: seq B) x1, x \in x1 -> x \in x0 ++ x1.
Proof.
intros. rewrite mem_cat H orbC //. 
Qed.
Hint Resolve sub_cat sub_cat2.

Lemma co_eq_derive : forall (c0 c1 : regex) l e,(forall x y, (x,y) \in l -> x =(nil)=  y) ->  c0 =(l)= c1 -> e \ c0 =( l)= e \ c1.
Proof.
move=> c0 c1 /= l e Hl Heq. 
move: Heq e Hl. elim;intros;subst;try solve [ simpl; rewrite ?eqs;eauto] .
...
-  rewrite /=; case Hcase: (nu _)=>//;rewrite !eqs //. eq_m_left.  eauto.
(*  rewrite c_plus_comm. eq_m_left.*)
-  rewrite /=. case Hcase:(nu c2)=>//=;case Hcase':(nu c3)=>/=//;rewrite !eqs;eq_m_left. eauto.
(*  rewrite {2}c_plus_comm -c_plus_assoc eqs c_plus_comm //. *)
-  rewrite /= eqs; case Hcase:(nu _)=>/=;rewrite  ?eqs //. 
- ssa. (*apply:c_plus_ctx. apply:c_ineq_mon. 2: eauto.*)
(*- move: (H0 e Hl). ssa. exists x. eauto.
- move: (H e Hl) (H0 e Hl). ssa. exists (x++x0). apply/c_trans. 
  apply:c_ineq_mon=>/=.  2: eauto. eauto. eauto.
- move: (H e Hl) (H0 e Hl). ssa.  exists (x++x0). apply:c_plus_ctx. eauto. eauto.
- move: (H e Hl) (H0 e Hl). ssa.  exists (x++x0++l0). *) simpl.
  have: nu c2 = nu c0' by apply/c_eq_nu; eauto. move=>->.
  case Hcase:(nu _)=>//=;subst. apply:c_plus_ctx. apply:c_seq_ctx. apply:H. done. done. eauto.
(*   apply:c_ineq_mon. 2: eauto. ssa. eauto. apply:c_ineq_mon=>/=. 
   2: {  apply c_seq_ctx. eauto.  apply:c_ineq_mon.  2:eauto. eauto. } 
   eauto. *)
- simpl. apply:c_seq_ctx. eauto. done. (*apply:c_ineq_mon=>/=. 2:eauto. eauto.*)
- rewrite /=. case Hcase: (_==_)=>//. norm_eqs. rewrite !eqs. apply:c_ineq_mon=>/=. 2: eauto. done.
- rewrite !eqs //.
- rewrite /=. case Hcase: (_==_)=>//. norm_eqs. rewrite !eqs. apply:c_ineq_mon=>/=. 2: eauto. done.
...
- apply:c_add.
move: (H0 e Hl). ssa. exists (x++l0). apply:c_seq_ctx. eauto. eauto.*)

Lemma co_eq_derive : forall (c0 c1 : regex) l e, (*(forall x y , (x,y) \in l -> x =(nil)=  y)*) 
                                            c0 =(l)= c1 ->  e \ c0 =(l)= e \ c1.
Proof.
move=> c0 c1 /= l e Heq. 
move: Heq e. elim;intros;subst;try solve [ simpl; rewrite ?eqs;eauto] .

- exists nil. rewrite /=. case Hcase: (nu c2)=>//=; case Hcase1: (nu c3)=>//=; rewrite !eqs;eq_m_left. 
- exists nil. rewrite /=; case Hcase:(nu _)=>//=; rewrite !eqs //.
- exists nil. rewrite /=; case Hcase: (nu _)=>//;rewrite !eqs //.  
- exists nil. rewrite /=; case Hcase: (nu _)=>//;rewrite !eqs //. eq_m_left. eauto.
(*  rewrite c_plus_comm. eq_m_left.*)
- exists nil. rewrite /=. case Hcase:(nu c2)=>//=;case Hcase':(nu c3)=>/=//;rewrite !eqs;eq_m_left. eauto.
(*  rewrite {2}c_plus_comm -c_plus_assoc eqs c_plus_comm //. *)
- exists nil. rewrite /= eqs; case Hcase:(nu _)=>/=;rewrite  ?eqs //. 
- move: (H0 e Hl). ssa. exists x. eauto.
- move: (H e Hl) (H0 e Hl). ssa. exists (x++x0). apply/c_trans. 
  apply:c_ineq_mon=>/=.  2: eauto. eauto. ep. 
- move: (H e Hl) (H0 e Hl). ssa.  exists (x++x0). apply:c_plus_ctx. eauto. eauto.
- move: (H e Hl) (H0 e Hl). ssa.  exists (x++x0++l0). 
  have: nu c2 = nu c0' by apply/c_eq_nu; eauto. move=>->.
  case Hcase:(nu _)=>//=;subst. apply:c_plus_ctx. apply:c_seq_ctx. eauto. eauto. eauto.
  apply:c_seq_ctx. eauto. eauto.
- move: (H0 e Hl). ssa. exists (x++l0). apply:c_seq_ctx. eauto. eauto.
- have:  (forall x y : regex, (x, y) \in (r, r') :: l0 -> x = ([::])= y).
  intros. rewrite !inE in H1. de (orP H1). norm_eqs. inv H2.
apply/H0. intros. rewrite !inE in H1. de (orP H1). norm_eqs. inv H2.
eauto.  eauto. 
...
Definition proj_sig {A : Type} {P : A -> Prop}  (H : { x| P x}) : A :=
match H with 
| exist x _ => x
end.
