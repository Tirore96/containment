...
Require Export Containment.utils.
(*From Sub.Process Require Import preds.*)
...
Theorem inc_increments : forall l, inc l = map (fun x => x +1) l.
Proof. 
...
Proof. 
intros. rewrite /map_keys. Search _ (map _ ?l = ?l). rewrite -{2}(@map_id _ l) . 
...
Lemma lookup_map_aux2 : forall A B C (l : seq (A * B)) (k : A) (f : A -> C), injective f -> 
                                                                lookup l k = lookup (map_keys f l) (f k).
Proof. 
move => A B. intros. move : H. 
...
Lemma lookup_map_eq_inj : forall A B C (l : seq (A * B)) (k : A) k' (f : A -> C), injective f -> f k = k' ->
                                                             lookup (map_keys f l) k' =    lookup l k.
Proof. 
...
Lemma map_keys2 : forall (A B C D : eqType) (l : seq (A * D)) (fm : A -> B) (f : B -> C), map_keys f (map_keys fm l) = map_keys (f \o fm) l. 
Proof. 
...
Lemma in_update2 : forall (A B : eqType) (l : seq ( A * B)) x (k : A)  v, x \in update l k v -> x = (k,v) \/ x \in l.
Proof. 
move => A B. elim;ssa.
